...
function showProfile() {
  if (localStorage.getItem("username") == null) {
    ...
    if (localStorage.getItem("highscore") == null) {
      ...
      document.getElementById("profileHighscore").innerHTML =
...
function clearUserData() {
  var confirmation = prompt(
    "Are you sure you want to clear all user data? Type 'CONFIRMCLEARDATA' to confirm."
  );
...
function gameRound() {
  ...
  document.addEventListener("keydown"),
    function (input) {
      if (
        (input.key == "ArrowLeft" && word == colour) ||
...
function verifyRound(input, round) {
  ...
  var colour = round[1];
  var validFlag = 0;
  if ((input == "ArrowLeft" && word == colour) || (input == "ArrowRight" && word != colour)) {
    validFlag = 1;
  } else if (
    (input == "ArrowLeft" && word != colour) ||
